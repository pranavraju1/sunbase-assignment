...
function getDragAfterElement(main, y) {
  //logic to get next element while dragging
  const draggableElements = [
    ...main.querySelectorAll(".input-item:not(.dragging)"), //elements not being dragged
  ];
  return draggableElements.reduce(
    //to find the one closest to current mouse position
    (closest, child) => {
      //closest is the accumulator and child are the elements
      const box = child.getBoundingClientRect();
      // DOMRect object providing information about the size of an element and its position relative to the viewport.
      // console.log(box);
      const offset = Math.floor(y - box.top - box.height / 2); //calculating the distance b/w mouse and center of child
      console.log(offset);
      if (offset < 0 && offset > closest.offset) {
        //if less than zero element hovering on elemnts abovve
        return { offset: offset, element: child };
      } else {
        //else element is hovering on elemnts below
        return closest;
      }
    },
    { offset: Number.NEGATIVE_INFINITY }
    //closest initial value is negative infinity this is done to ensure that
    // the  first draggable element encountered will always be considered closer initially.
  ).element;
}
...
form.addEventListener("click", (e) => {
  ...
  if (inputItems.length === 0) {
    //checking if any element is added in form
